...
def customize_resume():
    try:
        ...
        output_file, added_keywords = updater.update_resume(requirements_text)
        if not output_file or not os.path.exists(output_file):
            shutil.rmtree(temp_dir)
            return jsonify({'success': False, 'message': 'Failed to process resume'})
        shutil.rmtree(temp_dir)

        # Create message with added keywords
        message = 'Resume customized successfully'
        if added_keywords:
            message += f' - Added {len(added_keywords)} new skills: ' + ', '.join(added_keywords[:10])
            if len(added_keywords) > 10:
                message += f' and {len(added_keywords) - 10} more'
...
def download_resume(filename):
    try:
        ...
        return send_file(file_path, as_attachment=True, download_name=filename, mimetype='application/vnd.openxmlformats-officedocument.wordprocessingml.document')
    except Exception as e:
